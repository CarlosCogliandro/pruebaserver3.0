...
class UserController {
    ...
    try {
      ...
      if (!first_name || !email || !age || !password) {
        const customeError = new CustomeError({
          ...
          cause: generateUserError({ first_name, last_name, email, avatar, age, password, rol }),
  ...
  async uploadPremiumDocuments(req, res) {
    try {
      ...
      if (files.identificationDocument) { updateOrAddDocs("identificationDocument", files.identificationDocument[0]); }
      if (files.domicileProofDocument) { updateOrAddDocs("domicileProofDocument", files.domicileProofDocument[0]); }
      if (files.accountStatementDocument) { updateOrAddDocs("accountStatementDocument", files.accountStatementDocument[0]); }
      await user.save();
  ...
  async updateUserPremium(req, res) {
    try {
      ...
      const hasAllDocuments = requieredDocs.every((docName) => user.documents.some((doc) => doc.name === docName && doc.status === "Uploaded"));
      if (hasAllDocuments) {
        user.isPremium = true;
        user.rol = "premium";
  ...
  async getUserManagment(req, res) {
    ...
    admin = (user.rol === "admin") ? true : false;
    premium = (user.rol === "premium") ? true : false;
    users = await this.userService.getAll();
    res.render("userAdministration", { users, user, admin, premium, active: { userM: true }, })
  };
  ...
  async deleteInactiveUsers(req, res) {
    try {
      ...
      const usuariosInactivos = await userModel.find({ last_connection: { $lt: fiveMA } })
