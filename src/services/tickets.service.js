...
import TicketContainer from "../dao/mongoDB/ticketContainer.js"
...
class TicketService {
  ...
  async getTicketByOnlyCode(code) {
    ...
    const tickets = await this.ticketContainer.getTicketByCode(code);
    return tickets;
  };

  async createTickets(data) {
    const { total, purchaser } = data;
    return this.ticketContainer.createTicket({ amount: total, purchaser: purchaser });
  };

  async getTicketById(tid) {
    if (!tid) throw new Error('Ticket ID is required.');
    const tickets = await this.ticketContainer.getTicketById(tid);
    return tickets;
  };
};
